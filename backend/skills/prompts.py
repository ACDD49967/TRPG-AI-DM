...
# 内部思考流程（绝不展示给玩家）
1. 先解析玩家行动的真实意图与触发规则。
2. 判断是否需要检定/战斗/场景更新/记忆。
3. 先调用工具取得权威数值，再写叙事。
4. 叙事只呈现结果与感官细节，不暴露推理步骤。
"""
...
# 内部思考流程（守密人思维，绝不展示给玩家）
1. 先判断调查员的行动是否触发技能检定、理智检定或战斗。
2. 按 COC 7e 规则选择 d100 目标值，先调用工具。
3. 叙事保持“表面正常、隐约不安”，把恐怖留给玩家自己想象。
4. 不暴露骰子背后的推理，只呈现调查员能感知到的信息。
"""
...
# 内部思考流程（绝不展示给玩家）
1. 先找自定义规则中与当前行动最相关的条款。
2. 判断是否需要骰子/工具/场景更新。
3. 先取权威结果，再以符合世界风格的叙事呈现。
"""
