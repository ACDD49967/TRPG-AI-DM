...
- 法术图鉴内提供“翻译 SRD 法术”按钮：使用当前会话配置的 LLM 按批次机翻 SRD 法术名称与描述为简体中文，前端实时显示进度条，翻译结果持久化并自动刷新
- 玩家与 DM 均可在游戏内自建法术（法术弹窗直接打开自建表单，字段与内置法术卡一致）；同名法术/地图/生物在同一剧本内重复新增时会自动更新而不是重复添加
- 地图/地点图鉴与生物图鉴对标法术书：统一折叠卡片、完整结构化字段（地点：类型/状态/文化/区域/知名人物/危险/秘密；生物：六维/AC/HP/技能/特性/动作/习性/栖息地/传说/弱点）、弹窗内直接自建、搜索覆盖全部字段；同名条目同剧本内自动更新；支持从知识库自动抓取地点/城市结构化条目，并提供“机翻地点/生物描述”按钮
- 地图、生物图鉴、法术图鉴均按剧本隔离；DM 可通过 `search_*` 低 token 精简查询，或 `get_bestiary_card` / `get_location_card` 获取完整卡面
- 世界生成时自动提取 NPC、地点、生物、法术进入图鉴
- NPC 分重要/简单两类：重要 NPC 生成完整角色卡（性格/动机/秘密/属性/特性/技能），简单 NPC 同样包含属性/技能/特性（简略版），只省略深层性格/动机/秘密；DM 可用 `promote_npc` 将简单 NPC 提升为重要 NPC
- 剧本导入支持三种切分：快速切分、语义切分、LLM 智能切分；知识库提供“LLM 智能切分与图鉴注入”按钮，自动从文档提取地点/生物/法术写入图鉴

### 角色与存档

- 角色卡独立于剧本保存，可复用；D&D 与 COC 使用对应官方纸面角色卡布局
- 角色卡职业资源、法术位、已习得法术、攻击/豁免/技能均展示计算公式与最终值；法术显示为「火球术：三环 塑能（术士、法师）」，点开查看施法时间/距离/成分/持续/完整效果
- 生物图鉴与 NPC 卡同样折叠显示：怪物摘要为「名称 · CR · HP · AC」，NPC 卡按 D&D 官方 NPC 卡排版（护甲等级/生命值/六维调整/技能/特性/动作）
- 角色创建支持按职业/种族选择戏法与一环法术：法师 3 戏法+6 法术、术士 4+2、牧师/德鲁伊按感知调整准备法术、诗人 2+4、邪术师 2+2；高等精灵额外获得法师戏法，提夫林获得奇术；SRD 法术已内置离线职业映射，同样按职业过滤可选
