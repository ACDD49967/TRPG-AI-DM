...
if ! "$PYTHON" -c "import fastapi,uvicorn,openai,sqlalchemy,aiosqlite,pydantic,dotenv" 2>/dev/null; then
    ...
    "$PYTHON" -m pip install fastapi httpx json-repair jieba rank-bm25 "uvicorn[standard]" anthropic "sqlalchemy[asyncio]" aiosqlite pydantic python-dotenv sse-starlette \
