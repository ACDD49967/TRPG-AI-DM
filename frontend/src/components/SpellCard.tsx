    ...
    <details className={`group rounded-lg border ${paper ? 'border-amber-900/20 bg-white/70' : 'border-gray-200 bg-white'}`}>
      <summary className="cursor-pointer select-none px-2.5 py-1.5 flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-ink-800">
          ...
          {spell.name_zh && spell.name !== spell.name_zh && <span className="text-ink-400 font-normal">（{spell.name}）</span>}
          {classes && <span className="text-ink-400 font-normal">（{classes}）</span>}
        </span>
      ...
      <div className="px-2.5 pb-2 border-t border-gray-100 text-[11px] leading-relaxed text-ink-700 space-y-1">
        {spell.casting_time && <p><span className="text-ink-400">施法时间：</span>{spell.casting_time}</p>}
        {spell.range && <p><span className="text-ink-400">施法距离：</span>{spell.range}</p>}
        {spell.components && <p><span className="text-ink-400">法术成分：</span>{spell.components}</p>}
        {spell.duration && <p><span className="text-ink-400">持续时间：</span>{spell.duration}</p>}
        {displayDesc && <p className="text-ink-700 whitespace-pre-line">{displayDesc}</p>}
        {spell.description_zh && spell.description && <p className="text-ink-400 text-[10px] italic whitespace-pre-line">原文：{spell.description}</p>}
      </div>
    </details>
  );
}
