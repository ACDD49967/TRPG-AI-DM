...
export default function CocInvestigatorSheet({ onClose, embedded = false }: { onClose?: () => void; embedded?: boolean }) {
    ...
    <div className={embedded ? 'text-ink-900' : 'paper-card rounded-xl max-w-3xl w-full max-h-[88vh] overflow-y-auto p-5 text-ink-900'}>
      ...
      <div className="flex items-start justify-between border-b-2 border-amber-900/30 pb-3">
        <div>
          ...
          <p className="text-xs text-ink-600 mt-1">{status.race || '调查员'} · {status.char_class || '未知职业'} · COC 7e</p>
        </div>
      ...
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mt-3">
        {[
          ...
          <div key={k} className="bg-amber-50/70 border border-amber-900/20 rounded-lg p-2 text-center">
            <p className="text-[9px] uppercase tracking-widest text-ink-500">{k}</p>
          ...
          {COC_ATTRS.map(([k, label]) => {
            ...
            return (
              <div key={k} className="bg-white border-2 border-amber-900/30 rounded-lg p-2 text-center shadow-sm">
                <p className="text-[9px] uppercase tracking-widest text-ink-400">{label}</p>
                <p className="paper-title text-2xl font-black">{v}</p>
                <p className="text-[9px] text-ink-500 mt-0.5">1/2: {Math.floor(v / 2)} · 1/5: {Math.floor(v / 5)}</p>
              </div>
            );
          })}
        </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-1.5">
            {Object.entries(status.skills).map(([k, v]) => (
              <div key={k} className="bg-white/70 border border-amber-900/20 rounded px-2 py-1 flex items-center justify-between">
                <span className="text-[10px] text-ink-600">{k}</span>
