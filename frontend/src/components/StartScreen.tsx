...
export default function StartScreen(){
  ...
  const [savedScenarios,setSavedScenarios]=useState<Array<{id:string;title:string;description:string;summary?:string;tone:string;score:number;total_sessions:number;character_name?:string;race?:string;char_class?:string}>>([]);
  ...
  const finalAttrs=useMemo(()=>aiGen?.attributes||attrs,[attrs,aiGen]);
  const rc=RACES[race];const cc=CLASSES[charClass];
  ...
  const start=async()=>{
    ...
    try{
      const r=await fetch('/api/game/new',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
        username:username||'冒险者',character_name:charName,gender,race:rc.name,char_class:cc.name,
        attributes:finalAttrs,race_traits:rc.traits,class_proficiencies:cc.profs,
        api_key:apiKey||undefined,model_name:modelName||undefined,base_url:baseUrl||undefined,
        ...
        new_world:!scenarioId,skill_proficiencies:skillPicks,
        play_mode:playMode,
      })});
        ...
        <div className="card p-6 space-y-5">
          ...
          {step===2&&(
            <div className="space-y-5">
              ...
              {/* 职业 */}
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-2">职业</label>
                <div className="grid grid-cols-2 gap-1.5">
                  {Object.entries(CLASSES).map(([k,v])=>(
                    <button key={k} onClick={()=>setCharClass(k)} className={`p-2 rounded-lg border text-left text-xs transition-all ${charClass===k?'border-indigo-400 bg-indigo-50 text-indigo-700':'border-gray-200 bg-white text-gray-600 hover:border-gray-300'}`}>{v.name} <span className="text-[9px] text-gray-400">({v.hd})</span></button>
                  ))}
                </div>
                <div className="mt-2 bg-gray-50 rounded-lg p-2.5 border border-gray-200">
                    ...
                    <div className="space-y-1 max-h-48 overflow-y-auto">
                      {savedScenarios.map(s=>(
                        <button key={s.id} onClick={()=>loadScenario(s.id)} className={`w-full text-left p-2.5 rounded-lg border text-xs transition-all ${selectedScenario===s.id?'border-indigo-400 bg-indigo-100 ring-1 ring-indigo-300':'border-gray-200 bg-white hover:border-gray-300'}`}>
                          ...
                          <div className="flex gap-3 mt-1 text-[10px] text-gray-500"><span>{s.tone}</span><span>游玩{s.total_sessions}次</span>{s.character_name&&<span>角色:{s.character_name}</span>}</div>
          ...
          {step===3&&(
            <div className="space-y-5">
              ...
              <div className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                <p className="text-[10px] text-gray-500 font-medium mb-2">角色预览</p>
                <p className="text-sm text-gray-800"><span className="font-bold text-indigo-600">{charName||'???'}</span><span className="text-gray-400"> — </span><span>{rc.name} {cc.name} Lv.1</span></p>
                <div className="flex gap-3 mt-1 text-[10px] text-gray-500"><span>❤️ HP:30</span><span>🛡️ AC:12</span><span>🪙 10G</span><span>{playMode==='lite'?'⚡精简模式':'🐉深度模式'}</span></div>
                {skillPicks.length>0&&<p className="text-[10px] text-gray-500 mt-1">技能: {skillPicks.join('、')}</p>}
              </div>
